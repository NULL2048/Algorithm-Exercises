...
public class Code01_SegmentTree {
    ...
    public static class SegmentTree {
        ...
        private void pushDown(int rt, int ln, int rn) {
            // 先检查当前位置是否有更新任务，再检查当前位置是否有累加任务
            // 我们每次处理更新任务的时候，就会将当时节点上所有的累加任务都给清空，因为之前的累加任务都会被更新任务覆盖
            // 但是如果出现了同时存在更新任何和累加任务的情况时，原因只有一个，那么久累加任务比更新任务更加靠后
            // 所以在这里我们要先处理更新任务，再处理累加任务，不能让更新任务再把累加任务覆盖了，累加任务在更新任务之后，累加任务是在更新之后的基础上完成的
