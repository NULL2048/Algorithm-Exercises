...
public class Code04_LongestCommonSubsequence {
    ...
    //    注意这里不能用str1[i] == str2[j]去向下递归，因为递归函数就是传入的i和j，入轨递归调用再传入i和j，那么整个递归就无法结束了
    ...
    public static int process1(char[] str1, char[] str2, int i, int j) {
        // 下面去讨论四种base case
        // 1、当i和j都等于0时，也就是str1和str2都只剩下一个字符
        if (i == 0 && j == 0) {
            // str1[0..0]和str2[0..0]，都只剩一个字符了
            // 那如果字符相等，公共子序列长度就是1，不相等就是0
            // 这显而易见
            return str1[i] == str2[j] ? 1 : 0;
            // 如果只有i等于0时，也就是只有str1剩下1个字符，但是str2还剩下多个字符
        } else if (i == 0) {
            // 这里的情况为：
            ...
            // 如果只有j等于0时，也就是只有str2剩下1个字符，但是str1还剩下多个字符
        } else if (j == 0) {
            // 和上面的else if同理
            ...
            // i和j都不等于0，str1和str2都剩下多个字符
        } else { // i != 0 && j != 0
            // 这里的情况为：
            // str1[0...i]和str2[0...i]，str1和str2都不只一个字符
            // 看函数开始之前的注释部分，这里就要开始分那三种情况了
            // p1就是可能性c)
    ...
    public static int longestCommonSubsequence2(String s1, String s2) {
        ...
        char[] str1 = s1.toCharArray();
        char[] str2 = s2.toCharArray();
        // 创建dp数组，这里是样本对应模型，二维下标分别表示str1的字符和str2的字符，也就是两个下表分别对应着不同的样本
        int N = str1.length;
        int M = str2.length;
        int[][] dp = new int[N][M];
        // 根据上面暴力递归的第一个basecase去做赋初值
        dp[0][0] = str1[0] == str2[0] ? 1 : 0;
        // 根据上面暴力递归第二个basecase赋初值
        for (int j = 1; j < M; j++) {
            dp[0][j] = str1[0] == str2[j] ? 1 : dp[0][j - 1];
        }
        // 根据上面暴力递归第三个basecase赋初值
        for (int i = 1; i < N; i++) {
        ...
        // 根据暴力递归中递归函数调用位置的入参来的值要返回哪个dp元素
        return dp[N - 1][M - 1];
    }
