...
public class Code02_LightProblem {
    ...
    public static int noLoopMinStep1(int[] arr) {
        ...
        // 选择二改变0的位置时，没用算上改变0位置按开关的次数，需要在最后再加1，这里先判断是不是系统最大，看能不能让所有灯都打开，如果确实可以，再去加1，避免溢出
    ...
    public static int process1(int[] arr, int nextIndex, int preStatus, int curStatus) {
        // basecase  当前来到最后一个开关的位置
        if (nextIndex == arr.length) {
            return preStatus != curStatus ? (Integer.MAX_VALUE) : (curStatus ^ 1);
        }
        // 没到最后一个按钮呢！
        // i < arr.length
        // 根据当前的状态来决定到底要不要按开关，有点贪心的意思
        // preStatus表示当前来到的位置cur的状态
        // 1、如果当前cur的灯是关着的，那么此时必须要按下开关改变这个位置的状态，否则以后就再也没有机会将其打开了。
        if (preStatus == 0) {
            // 按下开关
            // 改变当前位置灯的状态
            curStatus ^= 1;
            // 这个cur指的是下一次递归的当前位置，也就是当前的nextIndex位置，按下开关将nextIndex位置的状态改变
            int cur = arr[nextIndex] ^ 1;
            // 递归得到后续位置能让灯全亮的按开关次数
            int next = process1(arr, nextIndex + 1, curStatus, cur);
            // 如果next不是系统最大，就加1，需要将本次按开关的次数加进去
            return next == Integer.MAX_VALUE ? next : (next + 1);
            // 2、如果当前cur的灯是开着的，那么此时一定不能按下开关改变这个位置的状态，否则把这个灯关上之后，以后就再也没有机会将其打开了。
        } else {
            // 直接向后调用，不改变任何位置的状态
    ...
    // 2、无环改灯问题的迭代版本。  这个时间复杂度和递归版本是一样的
    ...
    public static int loopMinStep1(int[] arr) {
        ...
        // 将0位置和1位置所有的装态度和都作为递归起点调用一遍，收集上来的四个答案取最小值
        // 0不变，1不变
    ...
    // 【设计简化外部信息的动态规划】  一定要学会用简单的数据类型来表示外部信息。这里引入的int endStatus, int firstStatus就很巧妙地描述了外部信息（环形）
    // 下一个位置是，nextIndex
...
//		}
    ...
    // for test 生成长度为len的随机数组，值只有0和1两种值
