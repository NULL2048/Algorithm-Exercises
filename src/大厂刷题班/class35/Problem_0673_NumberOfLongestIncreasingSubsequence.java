...
public class Problem_0673_NumberOfLongestIncreasingSubsequence {
    ...
    public static int findNumberOfLIS1(int[] nums) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            // 即使下面的循环找不到比自己小的数，以nums[i]结尾的递增序列的长度至少是1，这里之所以可以将preLen初始化为0，是因为在最后有一个preLen + 1的操作，能保证lens[i]至少为1
            int preLen = 0;
            // preCnt表示的是能和nums[i]组成递增子序列的最长递增子序列的总数量，初始化为1
            // 这道题说了要严格递增，也就是说相等的序列不算是递增序列。但是至少数字自己可以组成一个长度为1的递增序列，
            // 所以即使下面的循环找不到比自己小的数，以nums[i]结尾的递增序列的个数至少是1，也就是长度为1的一个递增子序列，所以这里要将preCnt初始化为1
            int preCnt = 1;
            ...
            for (int j = 0; j < i; j++) {
                // 如果nums[i]都不大于nums[j]，那么它就不可能与以nums[j]结尾的递增子序列组成一个新的递增子序列，这种情况直接跳过（因为题目要求严格递增，所以这里nums[j] == nums[i]的情况也要直接跳过）
