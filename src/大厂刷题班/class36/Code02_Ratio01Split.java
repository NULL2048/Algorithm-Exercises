...
public class Code02_Ratio01Split {
    ...
    public static int[] split(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // 如果此时的前缀仍是存在有0或者1一个都没有，那么划分数就很明确了，有多少个数就划分几部分，题目要求尽可能多的划分，每一部分只有一个相同的书01比例自然相同，满足题目要求
            if (zero == 0 || one == 0) {
                // 0~i前缀最多划分i+1部分
                ans[i] = i + 1;
                // 0和1，都有数量 -> 最简分数
                ...
                int b = one / gcd;
                // 当前0~i整体的01比例最简为a / b
                // 定理一：如果此时整体的01比例为a/b，想要对其进行划分，使得每一部分的01比例都一样的话，那么每一部分的01比例必须要和整体的01比例一样才行，这是一条定理
                // 定理二：并且举个例子如果j<i，0~j的01比例是a/b，整体0~i的01比例是a/b，那么j+1~i的01比例一定也是a/b，这也是一条定理
