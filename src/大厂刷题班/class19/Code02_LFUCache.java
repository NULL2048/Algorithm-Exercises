...
public class Code02_LFUCache {
    class LFUCache {
        // 桶内节点类   同一个桶内的节点他们的使用次数times都是一样的，使用双向链表连接，桶内节点的双向链表再按照最久未使用的时间来进行排列，最新使用过的在头部，最久未使用的在尾部
        // 节点的数据结构
        public class Node {
            public Integer key;
        ...
        public class NodeList {
            ...
            // NodeList类中的方法都是处理桶内Node节点的操作

            // 把一个新的节点加入这个桶，新的节点都放在桶内部链表的顶端，变成新的头部，头插法可以避免判断链表为空的情况
        ...
        // 整个结构中位于最左边的桶，也就是桶链表的头节点，就是当前存在节点的使用次数最小的桶
        ...
        // LFUCache类中的方法都是处理桶链表中桶节点的操作

        // 最重要也是最难点的方法就是modifyHeadList和move，这也是明显和LRU不同的地方
        // 这个方法是伴随着删除Node节点操作的
        // 这个函数的功能是，判断刚刚减少了一个节点的桶是不是已经空了，如果空了，需要去做桶链表的删除节点的操作（必须删除，因为我们需要保证桶链表的头部桶是有效的，需要随时从里面取出Node节点，以实现时间复杂度O(1)的要求）。
        // 记住删除桶节点的操作一定是和删除桶内部节点操作绑定在一起的，删除了桶内部节点之后不一定会触发删除桶节点操作，但是如果没有删除桶内部节点，就一定不会有删除桶节点的操作。
        // 1）如果桶内不空，则什么也不做
        //
        // 2) 如果桶内空了，并且removeNodeList还是整个缓存结构最左的桶(headList)。
        // 删掉这个桶的同时也要让最左的桶变成removeNodeList的下一个。
        //
        // 3) 如果桶内空了，removeNodeList不是整个缓存结构最左的桶(headList)。
        ...
        private boolean modifyHeadList(NodeList removeNodeList) {
            // 判断removeNodeList这个桶在删除了一个内部节点之后是否为空，如果为空就去要做桶链表删除该桶节点的操作
