...
public class Code01_NCardsABWin {
    ...
    public static double p1(int cur) {
        // basecase 胜利返回100%胜率的概率
        if (cur >= 17 && cur < 21) {
            return 1.0;
        }
        // basecase 输了，返回0%胜利的概率
        if (cur >= 21) {
            return 0.0;
        }
        // 向后递归
        double w = 0.0;
        // 尝试获取每一个数字，将计算出当前累加和获胜的概率，并把抽每一个数字的获胜概率累加
        for (int i = 1; i <= 10; i++) {
            w += p1(cur + i);
        }
        // 乘以1/10，就是cur这个累加和可能获胜的概率
        return w / 10;
    }
    ...
    public static double p3(int cur, int N, int a, int b) {
        ...
        // 只有cur < b - 1 - N时，通式才成立，否则w就只等于p3(cur + 1, N, a, b) + p3(cur + 1, N, a, b) * N，不用减p3(cur + 1 + N, N, a, b)
        if (cur + 1 + N < b) {
            w -= p3(cur + 1 + N, N, a, b);
        }
        return w / N;
