...
public class Code04_Drive {
    ...
    // A区域还有rest个名额!   这里只用A的剩余名额就行了，因为B的可以用A的求出来
    // 返回把index...司机，分配完，并且最终A和B区域司机数量同样多的情况下，index...这些司机，整体收入最大是多少！
    public static int process1(int[][] income, int index, int rest) {
        ...
        // 情况一：如果现在剩下的司机数量正好是A还剩余的名额，就让剩下的全部人都去A
        if (income.length - index == rest) {
            return income[index][0] + process1(income, index + 1, rest - 1);
        }
        // 情况二：如果A已经没有名额了，就只能让剩下的全部人都去B了
        if (rest == 0) {
            return income[index][1] + process1(income, index + 1, rest);
        }
        // 情况三：两种选择，当前司机可以去A，或者去B
        int p1 = income[index][0] + process1(income, index + 1, rest - 1); // 去A
        int p2 = income[index][1] + process1(income, index + 1, rest); // 去B
        // 返回收入最大值
    ...
    public static int maxMoney2(int[][] income) {
        ...
        // 自底向上去赋值
        for (int i = N - 1; i >= 0; i--) {
            // 从左到右或从右到左都可以，因为每一层都依赖自己下面一层的数据
            for (int j = 0; j <= M; j++) {
                if (N - i == j) {
